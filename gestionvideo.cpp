...
int GestionVideo::extractAudio(std::string filenameOut)
{
    ...
    av_register_all();
    avfilter_register_all();
    if ((ret = open_input_file_extract_audio(filename.c_str())) < 0)
    {
        freeMemory(&packet, frame, filter_ctx);
        return 0;
    }
    if ((ret = open_output_file_audio(filenameOut.c_str())) < 0)
    {
        freeMemory(&packet, frame, filter_ctx);
        return 0;
    }
    if ((ret = init_filters_audio()) < 0)
    {
        freeMemory(&packet, frame, filter_ctx);
        return 0;
    }
    bool stop = false;
    /* read all packets */
    while ((ret = av_read_frame(ifmt_ctx, &packet)) >= 0 && !stop) {
        stream_index = packet.stream_index;
        type = ifmt_ctx->streams[packet.stream_index]->codec->codec_type;
        //av_log(NULL, AV_LOG_INFO, "Demuxer gave frame of stream_index %u\n",stream_index);
        if (filter_ctx[0].filter_graph && stream_index == 1) {
            ...
            if (!frame) {
                ...
                if (ret < 0) {
                    av_frame_free(&frame);
                    av_log(NULL, AV_LOG_INFO, "Decoding failed\n");
                    stop = true;
                }
                else{
                    if (got_frame) {
                        frame->pts = av_frame_get_best_effort_timestamp(frame);
                        ret = filter_encode_write_frame(frame, 1, 0);
                        av_frame_free(&frame);
                        if (ret < 0){
                            freeMemory(&packet, frame, filter_ctx);
                            return 0;
    ...
    if (filter_ctx[0].filter_graph){
        ret = filter_encode_write_frame(NULL, 1, 0);
        if (ret < 0) {
            av_log(NULL, AV_LOG_INFO, "Flushing filter failed\n");
            freeMemory(&packet, frame, filter_ctx);
            return 0;
        }
        ...
        ret = flush_encoder(0);
        if (ret < 0) {
            av_log(NULL, AV_LOG_INFO, "Flushing encoder failed\n");
            freeMemory(&packet, frame, filter_ctx);
            return 0;
        }
    }
